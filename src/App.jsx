...
// import LipaseWorkflow from './workflow'; // Ensure file exists
// import apiData from './tests.json'; 

// function App() {

//   const handleOutputClick = (step) => {
//     // Logic to open results
//     console.log("Opening output:", step);
//     alert(`Opening output for step: ${step.step_type}`);
//   };

//   return (
//     <div style={{ margin: 0, padding: 0, minHeight: '100vh', backgroundColor: '#f8fafc' }}>
//       {apiData && apiData.map((workflowData, index) => (
//         <LipaseWorkflow 
//           key={workflowData._id?.$oid || index}
//           data={workflowData} 
//           outputfun={handleOutputClick} 
//         />
//       ))}
//     </div>
//   );
...
const parseCustomJson = (jsonArray) => {
  ...
  // Helper: group loose pads into components by ID
  const compMap = {};

  jsonArray.forEach(item => {
    // 1. PADS (SMD or Through-Hole)
    if (item.type === 'pcb_smtpad' || item.type === 'pcb_thpad') {
        const compId = item.pcb_component_id || 'misc_pads';
        if (!compMap[compId]) {
            compMap[compId] = {
                ref: 'CMP', 
                x: 0, y: 0, rot: 0, // Absolute pads don't need parent transform usually
                pads: [], 
                layer: 'F.Cu'
            };
        }
    ...
    // 3. TRACES (If they exist in this format as segments)
    ...
    reader.onload = (evt) => {
      ...
      try {
          ...
          if (Array.isArray(json)) {
              // Array format (PModBoard_circuit.json)
              parsed = parseCustomJson(json);
          } else if (json.footprints || json.segments) {
              // Internal format
              parsed = json;
              // Bounds fix if needed
              if (parsed.minX === undefined || parsed.minX === Infinity) {
                // (bounds calc logic omitted for brevity, usually handled by import)
